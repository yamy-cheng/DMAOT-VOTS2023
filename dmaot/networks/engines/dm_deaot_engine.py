...
from networks.engines.dm_aot_engine import DMAOTEngine, DMAOTInferEngine


class DMDeAOTEngine(DMAOTEngine):
    def __init__(self,
                 aot_model,
                 gpu_id=0,
...
class DMDeAOTInferEngine(DMAOTInferEngine):
    def __init__(self,
                 aot_model,
                 gpu_id=0,
                 long_term_mem_gap=9999,
    ...
    def add_reference_frame(self, img, mask, obj_nums, frame_step=-1):
        ...
        while (aot_num > len(self.aot_engines)):
            new_engine = DMDeAOTEngine(self.AOT, self.gpu_id,
